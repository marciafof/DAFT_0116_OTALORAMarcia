...
CREATE TABLE energy_generation_pcap
SELECT country, iso_code, year, population,
	electricity_demand, 
    per_capita_electricity as electricity_generation, 
    fossil_elec_per_capita as fossil_electricity, 
    nuclear_elec_per_capita as nuclear_electricity, 
    renewables_elec_per_capita as renewables_electricity, 
    other_renewables_elec_per_capita as other_renewable_electricity
FROM open_energy_data_clean
WHERE electricity_demand IS NOT NULL
	AND electricity_generation IS NOT NULL
    AND iso_code IS NOT NULL
...
PER CAPITA
*/ 
CREATE TABLE main_tb_raw_pcap
WITH energy_gen_rd_pcap AS (
	SELECT nrj.*, rd.rd_product,rd.efficiency, rd.total
	FROM energy_generation_pcap as nrj
	LEFT JOIN rd_country_budgets as rd
	ON nrj.country = rd.country
    AND nrj.year = rd.year
    )
SELECT energy_gen_rd_pcap.*, wbg_data.urbanpop, 
		wbg_data.co2_emis, wbg_data.incomeLevel,
        wbg_data.deltapop
FROM energy_gen_rd_pcap
LEFT JOIN wbg_data
ON energy_gen_rd_pcap.iso_code = wbg_data.id
	AND energy_gen_rd_pcap.year = wbg_data.year
;
